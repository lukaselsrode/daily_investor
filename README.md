...
| `odte-position` | **Broker-AWARE, DECISION-ONLY live-position watchdog — places NO orders, makes NO broker/LLM calls.** The discipline layer for an already-open 0DTE option: reads the active trade plan (`data/odte/active_trade.json`) + a **caller-supplied** live snapshot (Hermes feeds real broker/market values from its MCP tools — this command never fabricates broker data) and emits structured triggers `TAKE_PROFIT` (scale at +35–50%, strong full exit at +60%; thresholds accept fraction `0.20` **or** percent `20`) / `BID_MEMORY_PROTECT` (giveback harvest: once the best-seen bid arms at +15% over entry, a fade through 40% giveback of the best-seen gain fires `harvest_now`; the caller persists `management.best_seen_bid` between polls) / `THESIS_DEAD` (underlying/SPY/QQQ/IWM/VIX/VIXY stop levels; a `0` level is absent, never a live stop) / `BID_FLOOR` (near-worthless; falls back to `risk_rules.initial_bid_floor`) / `TIME_RISK` (`tighten_after` / `flat_before` ET) / `MONITORING_DEGRADED` (can't value the position) / `HOLD` / `NO_POSITION`. Employer-restricted underlyings (NVDA) return `RESTRICTED` with no management triggers. Writes `data/odte/{position_state,position_decision}.json`; **empty stdout** on `HOLD`/`NO_POSITION`, compact JSON on an actionable decision. Supply live values with `--snapshot PATH` or `--snapshot-json '{...}'`; `--plan` / `--state-dir` override defaults; `--json` always prints. The decision core (`evaluate_position`) is pure and unit-tested without Robinhood/network. |
...
| `odte-candidate-watch` | **PURE/OFFLINE pre-entry candidate HAWK lane — NO broker, NO LLM, NO network, places NO orders.** Watches a potential ETF/index setup until `CONFIRM_ENTRY`, `KEEP_WATCHING`, `DEGRADED_NO_TRADE`, `EXPIRED_NO_CONFIRMATION`, or `BROKER_BLOCKED`. Supports SPY/QQQ/XSP/IWM tape-only momentum candidates from VWAP/ORB/VIXY inputs, so morning ETF/index calls/puts can become first-class candidates without requiring Reddit/social. When vehicle score supplies a contract, candidate-watch locks its option ID, expiration, strike, and type into the candidate fingerprint; any contract switch starts a new identity cycle. Confirmation is **tiered** (2026-08-02 retune): **A+** = confirmed + ≥3 ETF confirmers with zero definitive dissenters; **B+** = confirmed + ≥2 confirmers with ≤1 definitive dissenter — tradeable on CHOP days at half size; partial tape (e.g. XSP snapshots without VWAP fields) is neutral, never a silent dissenter. At `CONFIRM_ENTRY` the candidate is stamped with its `tier` and the chase-band `anchor_quote` (contract ask) the lease price ceiling derives from. Only the **executable universe** (SPY/QQQ/IWM) can convert; XSP tape confirms but never converts. A confirmed candidate remains scan-only by itself; `--write` stores `data/odte/{active_candidate,candidate_decision}.json`. The fresh identity-matched `candidate_decision.json` is the only safe input that may cross the scan→gate boundary; the preferred conversion path is `odte-convert` (atomic re-check → gate → lease in one process). It does not mint a lease or place an order by itself. |
| `odte-entry-gate` | **PURE/OFFLINE gate evaluator — NO broker/network/LLM, places NO orders.** **LEGACY-PATH HARD BLOCK (2026-08-03): a `CONFIRM_ENTRY` candidate decision fed to this CLI/Make wrapper is REFUSED with `use_odte_convert` (exit 2, nothing journaled) — conversion runs ONLY via `odte-convert`, which calls the pure gate function in-process.** Non-confirm inputs (triggers, KEEP_WATCHING decisions) evaluate normally: it consumes candidate, day score, vehicle score, broker snapshot, gamma context, and explicit live confirmations. A scan-only candidate becomes a non-scan execution gate only when the confirmation decision is ≤60 seconds old and its recomputed candidate fingerprint, symbol, direction, selected vehicle, option ID, expiration, strike, type, and candidate-cycle timestamp match exactly. Every otherwise-executable gate—including an already non-scan/legacy caller—requires literal-true chain, spread, and budget confirmations plus explicit broker flatness/order counts. Stale/future/missing/mismatched confirmation stays fail-closed; the deprecated bare `--promote-to-execution` flag remains dead. Inputs: `CANDIDATE_DECISION=` and `CONFIRMATIONS=` via Make, or `--candidate-decision` / `--confirmations` via CLI. The live path must use `JOURNAL=1`; journal append failure exits nonzero, and loop-status independently refuses `PROMOTED` when confirmation fields are absent or false. Day/vehicle gates are **tier-aware** (2026-08-02): CHOP passes with an A+/B+ candidate tier (B+ = half-size), and a WATCH vehicle passes for B+ when its raw score clears the configured floor — bare CHOP/WATCH still fail closed. The day's journal also enforces the **daily trade budget** (default 2 entries/ET-day, `daily_trade_budget_exhausted`) and a **post-close cooldown** (default 20 min, `post_trade_cooldown_active`); green-day re-entry **AUTO-ARMS (2026-08-03)** when the daily budget has a slot, the cooldown passed, the candidate's tape tier ranks ≥ the winning trade's tier (a_plus > full > b_plus; legacy trades count as full), and BP ≥ 1.5× the contract's own cost — reason code `green_reentry_auto_armed_tier`; the explicit `allow_reentry_after_green` override remains the manual path, and `green_reentry_auto_arm: false` restores manual-only. A passing gate still requires a lease (`odte-convert` or `odte-execution-authorize`) before any broker review/place. |
...
| `odte-loop-status` | **PURE/OFFLINE loop state machine — makes NO broker call, NO LLM, places NO orders.** One read-only surface that tells the live controller (Hermes/MCP) **where in the loop it is and which command runs next**, so `scan → thesis → entry → watch → exit → review` reads as one obvious cycle instead of a pile of independent tools. Summarizes the canonical `data/odte/` artifacts (`active_trade.json`, `position_decision.json`, `active_candidate.json`, `candidate_decision.json`, `triggers.json`, and the latest `entry_decision` / `postmortem` in `decision_journal.jsonl`) into the current **state** — `SCAN` (keep scanning → `odte-watchdog`) / `CANDIDATE` (non-restricted candidate or active candidate watch on the board, scan_only → `odte-candidate-watch`, or `odte-convert` when confirmed) / `GATED` (entry-gate record, not execution-allowed → refresh inputs, re-run `odte-entry-gate`, then `odte-execution-authorize`; bare promotion is dead) / `PROMOTED` (gate execution-allowed → mint a lease via `odte-execution-authorize`, broker review/place, then `odte-order-guard` → `odte-position`) / `PENDING_ORDER` (an ENTRY order is live at the broker or must be cancelled — the order-guard lane outranks all scan/new-entry work; postures `PENDING_ORDER_GUARD` / `CANCEL_PENDING_ORDER`) / `ENTERED` (plan open, no live decision yet) / `MANAGING` (live position + current decision → `odte-position`) / `EXITED` (trade closed, no postmortem → `odte-journal` then `odte-ingest-artifacts`) / `REVIEWED` (closed + reviewed → `odte-journal-report`) / `DEGRADED` (live position can't be valued, or a live artifact is malformed/stale) — plus the **next command**, an `executable` flag, and a short `reasons` list. For the cron/Telegram lane it also emits a coarse **`posture`** (one word for what to DO this tick — `MANAGE_POSITION` / `SCOUT_FRESH_SETUP` / `WAIT_FRESH_CONFIRMATION` / `FLAT_NO_TRADE` / `STALE_DATA_BLOCKED` / `BROKER_DEGRADED`; **`FLAT_NO_TRADE`** is the normal idle tick — flat / reviewed / no candidate / empty heartbeat — and never contains the word "stale", while **`STALE_DATA_BLOCKED`** is reserved for an actual stale/malformed artifact *blocking* action, e.g. an open position whose live read went stale, never a leftover ignored after a review), a **`notes`** list (housekeeping/audit breadcrumbs kept out of the user-facing `reasons`), per-artifact **`artifact_ages`** (as-of / age / TTL / fresh for triggers, candidate, gate, position_decision, active_trade — so a stale candidate/gate is never recycled as fresh edge), and a normalized **`broker_lane`** from a **supplied/probed** broker-health payload (**this command never touches the broker**): `ok` / `down` / `stale` / `read_only_fallback` / `unknown` with an `orders_ok` flag. The **durable handoff** is a canonical `data/odte/broker_health.json` that cron/Hermes writes each tick from its MCP/CLI/read-only probes (auto-folded with no flag; override with `--broker-health PATH` or the `ODTE_BROKER_HEALTH` env; a file older than ~5 min fails closed → `stale`, orders blocked). Canonical schema (see `data/odte/broker_health.example.json`): `parent_robinhood_mcp` `OK`/`DOWN`/`UNKNOWN` (the **authoritative** review/place lane), `cli_mcp_test` `CONNECTED`/`DOWN`, `fallback` (local read-only probe), `live_review_place_allowed` (master order switch), `as_of`/`source`. The **split-brain rule**: a `CONNECTED` CLI `hermes mcp test robinhood` or a healthy read-only probe verifies flatness/BP/positions but does **not** authorize order review/place — so parent `DOWN` + CLI `CONNECTED` reads `read_only_fallback` and **live orders are BLOCKED** (posture `BROKER_DEGRADED`, `executable=false`), never `ok`. **Any** broker lane fault (`down`/`stale`/`read_only_fallback`) forces `BROKER_DEGRADED` as the top-line even when the loop is flat, so a cron tick can never imply the live lane is fine. The surface **fails closed by default** (live mode, the cron entrypoint): with a live position or a ready setup, an `unknown`/missing broker lane cannot authorize a live order and reads `BROKER_DEGRADED` — a live order is only authorized when `parent_robinhood_mcp` is a fresh `OK` and `live_review_place_allowed` is true. A *flat* tick with an unknown lane is just `FLAT_NO_TRADE` (nothing to authorize). `--offline` (`OFFLINE=1`) relaxes the fail-closed for pure decision-support. **Re-derives NO gate or decision** — it only reads what the other tools already wrote. A **live position always outranks** the scan/candidate/gate lane (managing/exiting an open trade beats chasing a new one); missing artifacts read as `SCAN` and a malformed/stale live artifact **degrades** rather than crashing. `--json` prints the compact machine payload (clean stdout contract); default prints Markdown; `--state-dir DIR` overrides `data/odte/`. Every payload also carries **`live_rails`** — the authoritative CURRENT numbers (conversion SLA, snapshot/lease TTLs, chase band, debit fractions AND computed dollar ceilings from fresh broker-truth BP — null when the probe is stale, never guessed — daily budget state, executable universe, green re-entry state), so the controller reads live policy every tick instead of enforcing remembered/retired rails (the 2026-08-03 $1.20-ghost-rail and hand-60s SLA failures). Post-green, the scan/watch lane now stays ALIVE when a budget slot is open and the cooldown passed (tier/BP enforced at the gate inside odte-convert); a below-winner tier is consumed early. And **`weekly_telemetry`** (ISO-week ET trades vs the 3–4/week target, conversion-funnel counts, top lease-refusal reasons, today's budget remaining) with a **zero-trade tripwire** that arms mid-week — a dying week is visible while it is still fixable. (`data.odte_loop_status.derive_loop_state()` is the pure, unit-tested core; `classify_broker_lane()` normalizes the health payload.) |
| `odte-convert` | **PURE/OFFLINE atomic conversion — CONFIRM_ENTRY → entry gate → execution lease in ONE process, places NO orders (2026-08-02 retune).** Born from the 2026-07-27..31 zero-trade week: orchestrating candidate-watch → entry-gate → authorize as separate commands across a multi-minute controller tick could not beat the 60-second freshness bounds (on Jul 31 the gate PASSED and the lease was refused 11s later on snapshots built earlier in the same tick). `odte-convert` runs the whole conversion under ONE clock: fresh tape re-check + re-anchor (candidate-watch) → **computed** final confirmations (`live_chain_recheck` = fresh identity-matched quote, `spread_cap_check` ≤ 25%, `budget_check` ≤ tier fraction × BP — deterministic, never agent-asserted) → entry gate → `authorize_entry`, so the tight in-process TTLs pass by construction while remaining fully enforced. Inputs: the active/confirmed candidate (default `data/odte/active_candidate.json`) plus **fresh** market/broker/contract snapshots fetched immediately before the call — the preflight refuses (naming the exact stale input) beyond the 120s snapshot TTL. Every non-converting stage journals an **identity-bound terminal `no_trade_decision`** (option_id + fingerprint + failed stage) — no silent scan-only dead ends; a journal-append failure withholds the lease (exit 2). Writes `candidate_decision.json` / `active_candidate.json` / `execution_lease.json` through the same atomic writers the individual CLIs use. The execution-safety invariants are UNCHANGED — single-use exact-identity lease, 60s hard cap, chase band, tiered sizing, order guard afterward. |
| `odte-execution-authorize` | **PURE/OFFLINE execution-lease authorization — the ONE tier that mints execution authority, places NO orders.** Born from the 2026-07-23 delayed-fill incident: a `scan_only` watchdog signal was promoted with a bare boolean and the stale 2-contract SPY 737P order filled minutes after the momentum died. Execution authority is now a **single-use, short-lived `ExecutionLease`** (config default **60s TTL**, configurable downward only, hard cap **60s** — the cap is not config-tunable) bound to one exact symbol / direction / option contract / quantity / **price ceiling** / **debit ceiling**, plus the candidate + market fingerprints it was authorized against. Inputs: `--gate`, `--candidate-decision`, `--vehicle-score`, `--broker`, `--market` (each PATH or `*-json`), `--policy` (risk mode / TTL / quantity / management plan). **Everything fails closed**: every input must carry a parseable timestamp inside its own TTL; the candidate decision must be a fresh `CONFIRM_ENTRY`; the gate must carry explicit `scan_only=false` + `execution_allowed=true` + boolean confirmations (strings/missing reject); identity must agree exactly (a QQQ thesis can never lease a SPY contract); broker truth must show BP, zero open positions/orders, today's order count, no block/lock. Default risk mode `PARTIAL_ACCOUNT` = **1 contract**, debit ≤ the tier's fraction of BP (**60% full tier / 30% B+ half-size tier**, 2026-08-02 retune), and a **chase band**: the fill ceiling is the CONFIRM_ENTRY `anchor_quote` × (1 + 15%) — re-anchored at confirmation so a premium that rose *because* the thesis confirmed no longer auto-vetoes (the incident order still fails: 2 contracts, 83.9% of BP). Market/broker snapshot TTLs are 120s (config); the in-process artifacts stay at 60s. `FULL_ACCOUNT_A_PLUS` permits full-account debits **only** with an explicit policy recording max premium loss, exact trigger, named invalidation, live bid/ask, target, scratch rail, and management cadence — a model-generated "A+" label alone never sets size. Leases are **single-use** (`consume_lease`, ledger `data/odte/consumed_leases.json`); expired/consumed leases reject. `--write` stores `data/odte/execution_lease.json`; `--journal` appends `execution_lease_issued`. |
| `odte-cleanup` | **Deterministic `data/odte/` sweep — the ONLY sanctioned cleanup, places NO orders (2026-08-03).** Cleanup used to be prose aimed at the controller LLM running ad-hoc `mv`; canonical loop state (incl. the single-use lease ledger) was archived 21× since Jul 2. This command owns the sweep with a **hardcoded keep-list** (`src/data/odte_cleanup.py`): canonical loop files (`triggers/watchdog_state/active_candidate/candidate_decision/execution_lease/consumed_leases/broker_health/decision_journal/active_state/position_*/active_trade/order_guard_decision/session_constants` plus the fast-lane canon `armed_intents/armed_intent_state/fast_lane_stage/fast_lane_status/fast_lane_pause`) and canonical dirs (`days/ reports/ scrape/ swarm/ precompute/ archive/ shadow/`) are untouchable by construction; everything else at the top level moves into ONE dated `archive/odte_cleanup_<ts>/` with a manifest. **Dry-run by default** (`APPLY=1` to act); `PRUNE_SCRAPE=1` trims timestamped scrape snapshots to `SCRAPE_KEEP` (default 100), never the stable pointers. |
| `odte-arm` | **The slow lane's ONLY sanctioned write path into `data/odte/armed_intents.json` — places NO orders (two-lane architecture, 2026-08-05).** An armed intent is a machine-readable standing order the LLM writes in advance: exact contract lock, trigger predicates (level acceptance with N-consecutive-check counters, ETF confirmations, VIXY condition, wall, ET fences), sizing HINTS (the lease stays authoritative), and the exit plan copied into `active_trade.json` on fill. Validation is **fail-closed at ARM time** (`data.odte_armed_intent.validate_armed_intent`): restricted underlyings (NVDA), non-executable vehicles (XSP), a same-ET-day 15:30 expiry ceiling, and malformed trigger blocks are refused with named reason codes — an intent that would be ambiguous at fire time never reaches the daemon. `INTENT=path` / `INTENT_JSON='{...}'` to arm, `DISARM=id\|all` (kill switch), `LIST=1`. |
| `odte-fast-lane` | **The deterministic fast-lane daemon — the ONLY component that may OPEN positions at `entries_live` (two-lane architecture, 2026-08-05).** Watches the tape at seconds cadence (5-min bars + live quotes via a direct MCP client with zero LLM turns), evaluates armed intents, and on trigger runs the ENTIRE existing chain in-process: tape re-check → `odte-convert` internals (budget/green/tier/chase band/lease) → ONE broker review → in-process pre-place guard (strictly stronger than the Hermes hook) → **consume-before-place** on the SHARED `consumed_leases.json` ledger → ref_id-idempotent place → order-guard poll (partial fills manage the FILLED quantity; incidents lock entries for the session) → `odte-position` exit management with bid-memory giveback + a bounded cancel/replace reprice loop. **The stage file `data/odte/fast_lane_stage.json` is the mode authority** (`shadow` → `exits_live` → `entries_live`; `make odte-fast-lane-stage STAGE=…`); flags can only be MORE conservative, a <48h broker token downgrades to shadow loudly, and in shadow mode the order tools are wrapped to RAISE — placement is structurally impossible, with every would-be action journaled to `data/odte/shadow/decision_journal.jsonl`. Kill switches: `make odte-fast-lane-pause` (pause file, next tick), `make odte-arm DISARM=all`, `make odte-fast-lane-uninstall` (launchd), stage rollback. Self-fences to 09:25–16:05 ET; heartbeat at `data/odte/fast_lane_status.json`. |
| `odte-shadow-report` | **PURE/OFFLINE shadow-vs-live divergence report — the rollout-gate adjudication surface (2026-08-05).** Joins the live decision journal with the fast lane's shadow journal (ET day + underlying, exact option_id preferred, ±5-min window fallback) into: matched pairs with **latency deltas** (negative = the shadow lane was faster) and limit-price deltas, `shadow_only` / `live_only` entry divergences, exit-timing divergences against `shadow_exit_intent`, incident tallies, and the Gate-1 **`clean`** flag that goes false on ANY unadjudicated divergence. Every divergence gets a one-line adjudication note before the next session; ≥2 clean shadow sessions gate exits-live, ≥2 more gate armed-entries-live. |
| `odte-order-guard` | **PURE/OFFLINE pending-order cancel-first state machine — places NO orders.** Consumes fresh **broker order truth** (`--order` / `--order-json`, caller-supplied — this repo never calls the broker), the execution lease (`--lease`, default `data/odte/execution_lease.json`), the current market snapshot, and the current time, and classifies: `NO_ORDER` / `PENDING_FRESH` (unfilled, lease alive, thesis rails valid) / `CANCEL_STALE_ENTRY` (**unfilled past the lease TTL — cancel IMMEDIATELY; a stale order is never extended**) / `CANCEL_THESIS_INVALID` (rails fired before the fill) / `FILLED_FRESH` (filled at/before lease expiry with exact identity → manage) / `FILLED_WITHOUT_VALID_LEASE` (**the 2026-07-23 incident path: a fill after lease expiry is an `execution_safety_incident` — journaled, new entries prohibited for the session, flatten/alert per the reviewed emergency policy — never "reclassified" as a normal position**) / `BROKER_MISMATCH_BLOCKED` (broker order disagrees with the lease identity/maximums). Exposes `pending_order_age_seconds` + `lease_seconds_remaining`; `odte-loop-status` surfaces this lane **above** all scan/new-entry work. A submitted order **burns its lease** in `data/odte/consumed_leases.json` (single use — concurrent controller ticks cannot double-submit). `--write` stores `data/odte/order_guard_decision.json`; `--journal` appends `entry_order_pending` / `entry_order_cancelled_stale` / `execution_safety_incident` / `execution_lease_consumed`. |
...
- `data/odte/{watchdog_state,triggers,active_candidate,candidate_decision,position_state,position_decision}.json` — watchdog/candidate/position state
- `data/odte/{execution_lease,order_guard_decision,consumed_leases}.json` — execution-safety layer: the single-use lease, the latest order-guard decision, and the consumed-lease ledger

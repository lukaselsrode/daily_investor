...
| `odte-candidate-watch` | **PURE/OFFLINE pre-entry candidate HAWK lane — NO broker, NO LLM, NO network, places NO orders.** Watches a potential ETF/index setup until `CONFIRM_ENTRY`, `KEEP_WATCHING`, `DEGRADED_NO_TRADE`, `EXPIRED_NO_CONFIRMATION`, or `BROKER_BLOCKED`. Supports SPY/QQQ/XSP/IWM tape-only momentum candidates from VWAP/ORB/VIXY inputs, so morning ETF/index calls/puts can become first-class candidates without requiring Reddit/social. A confirmed candidate is still `scan_only` / `execution_allowed=false`; the controller must build/promote a fresh `odte-entry-gate` and run broker review before any order. `--write` stores `data/odte/{active_candidate,candidate_decision}.json`. |
